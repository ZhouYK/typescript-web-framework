...
const PeriodicTable: FC<Props> = (_props: PropsWithChildren<Props>) => {
  ...
  const [renderer] = useState(() => {
    const tmpRenderer = new CSS3DRenderer();
  ...
  const onClickSphere = useCallback(() => {
    transform(targets.sphere, 2000);
  }, []);
  const onClickHelix = useCallback(() => {
  ...
  const elementRef = useCallback((i: number) => (ref: HTMLDivElement) => {
    // todo 这里ref可能为null
    const objectCss = new CSS3DObject(ref);
  ...
  const [elements] = useState(() => {
    ...
    for (let i = 0; i < table.length; i += 5) {
      const tmpEle = (
        <div key={i} ref={elementRef(i)} className='element' style={{ backgroundColor: `rgba(0, 127, 127, ${Math.random() * 0.5 + 0.25})` }}>
  ...
  return (
    <section className={style.periodic}>
      ...
      <section className='btn-row'>
        <button onClick={onClickTable}>表格</button>
        <button onClick={onClickSphere}>球体</button>
        <button onClick={onClickHelix}>环状螺旋</button>
        <button onClick={onClickGrid}>栅栏</button>
      </section>
    </section>
  );
};
